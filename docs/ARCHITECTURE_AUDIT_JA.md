...
- 監査日: 2026-07-14
- 更新日: 2026-07-16（自社作業員限定要件・健康診断日非収集方針を反映）
...
図の任意・必須カーディナリティは提案段階である。初期版の作業員は全員が自社所属で、作業員ごとの所属会社FKや会社選択を持たない。職種、現場の既定テンプレート、出力履歴・snapshotから現行マスターへの参照は、提出先が承認した項目、削除方針、保存要否を確認してからDB制約として確定する。
...
| `sites` | `organization_id`, `name`, `first_tier_company_name`, `first_tier_company_business_operator_id`, `own_construction_tier`, `address`, `start_date`, `end_date`, `manager_name`, `status`, `default_roster_template_id`, `archived_at`, 監査列 | **確定境界／候補項目**: 一次会社名・一次会社事業者ID・自社施工次数は現場情報。v9の表示用 `period` 文字列は日付へ分離 |
...
| `roster_templates` | `organization_id`, `name`, `source_name`, `active` | **提案**: 様式の論理的な系列。複数様式は初期必須ではない |
...
| 登録情報 | 現場一覧 | **確定領域／提案画面** | 通常の登録・修正は確定。稼働状態、一次会社名、自社施工次数、様式、不足、詳細の表示構成は提案 |
| 登録情報 | 現場追加・編集 | **登録修正は確定／項目・関連付けは提案** | 現場名、一次会社名、一次会社事業者ID、自社施工次数、住所、工期、担当者、既定テンプレートのうち、承認帳票へ必要な項目を確認 |
...
| 設定 | 職種・資格・様式 | **確認事項** | 初期版で管理UIを持つか、運用担当者と項目を確認。自社情報は登録情報で1社分だけ扱う |
...
| 中 | 将来、複数組織・組織切替・membershipを提供するか | テナントschema、RLS、管理UIが変わる。初期版には含めない |
...
- migrationで承認された所有境界、user、role、自社情報1社分、worker、site、assignment、templateの最小schemaを作る。協力会社・所属会社選択・複数名簿作成会社は含めず、複数組織membership・切替は必要性確認後に追加する。
...
- 現場一覧、追加、詳細、編集、一次会社名・一次会社事業者ID・自社施工次数・テンプレート関連付け。
- 現場別作業員情報の追加・編集。
